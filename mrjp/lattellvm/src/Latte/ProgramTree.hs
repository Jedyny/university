...
getPosS :: Stmt -> SourcePos
getPosS (AssignS pos _ _) = pos
getPosS (BlockS pos _) = pos
getPosS (CondS pos _ _ _) = pos
getPosS (DecS pos _ _) = pos
getPosS (ExpS pos _) = pos
getPosS (ForS pos _ _ _ _) = pos
getPosS (PredS pos _) = pos
getPosS (SuccS pos _) = pos
getPosS (ReturnS pos _) = pos
getPosS (WhileS pos _ _) = pos
getPosS s = error $ "Latte.ProgramTree.getPos: unsupported statement: " ++ show s  
